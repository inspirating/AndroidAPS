...
class DanaPenPacketApsSetEventHistoryTest : DanaPENTestBase() {
    ...
    private val packetInjector = HasAndroidInjector {
        AndroidInjector {
            ...
            if (it is DanaPENPacketAPSSetEventHistory) {
    ...
    @Test fun runTest() { // test for negative carbs
        ...
        var historyTest = DanaPENPacketAPSSetEventHistory(packetInjector, DanaPump.HistoryEntry.CARBS.value, now, -1, 0)
        var testParams = historyTest.getRequestParams()
        Assertions.assertEquals(0.toByte(), testParams[8])
        // 5g carbs
        historyTest = DanaPENPacketAPSSetEventHistory(packetInjector, DanaPump.HistoryEntry.CARBS.value, now, 5, 0)
        testParams = historyTest.getRequestParams()
        Assertions.assertEquals(5.toByte(), testParams[8])
        // 150g carbs
        historyTest = DanaPENPacketAPSSetEventHistory(packetInjector, DanaPump.HistoryEntry.CARBS.value, now, 150, 0)
        testParams = historyTest.getRequestParams()
