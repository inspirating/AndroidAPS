...
import app.aaps.pump.danapen.events.EventDanaPENDeviceChange
...
@Singleton
class DanaPENPlugin @Inject constructor(
    ...
    private var danaPENService: DanaPENService? = null
    ...
    override fun onStart() {
        super.onStart()
        val intent = Intent(context, DanaPENService::class.java)
        ...
        disposable += rxBus
            .toObservable(EventDanaPENDeviceChange::class.java)
            .observeOn(aapsSchedulers.io)
            .subscribe({
    ...
    private val mConnection: ServiceConnection = object : ServiceConnection {
        ...
        override fun onServiceConnected(name: ComponentName, service: IBinder) {
            ...
            val mLocalBinder = service as DanaPENService.LocalBinder
            danaPENService = mLocalBinder.serviceInstance
        }
    }
    ...
    override fun connect(reason: String) {
        ...
        if (danaPENService != null && mDeviceAddress != "" && mDeviceName != "") {
            val success = danaPENService?.connect(reason, mDeviceAddress) == true
            if (!success) ToastUtils.errorToast(context, app.aaps.core.ui.R.string.ble_not_supported_or_not_paired)
        }
    }

    override fun isConnected(): Boolean = danaPENService?.isConnected == true
    override fun isConnecting(): Boolean = danaPENService?.isConnecting == true
    override fun isHandshakeInProgress(): Boolean = false

    override fun disconnect(reason: String) {
        aapsLogger.debug(LTag.PUMP, "RS disconnect from: $reason")
        danaPENService?.disconnect(reason)
    }

    override fun stopConnecting() {
        danaPENService?.stopConnecting()
    }

    override fun getPumpStatus(reason: String) {
        danaPENService?.readPumpStatus()
    ...
    override fun loadHistory(type: Byte): PumpEnactResult {
        return danaPENService?.loadHistory(type) ?: instantiator.providePumpEnactResult().success(false)
    }

    override fun loadEvents(): PumpEnactResult {
        return danaPENService?.loadEvents() ?: instantiator.providePumpEnactResult().success(false)
    }

    override fun setUserOptions(): PumpEnactResult {
        return danaPENService?.setUserSettings() ?: instantiator.providePumpEnactResult().success(false)
    }
    ...
    override fun isBusy(): Boolean =
        danaPENService?.isConnected == true || danaPENService?.isConnecting == true
    ...
    override fun setNewBasalProfile(profile: Profile): PumpEnactResult {
        ...
        return if (danaPENService?.updateBasalsInPump(profile) != true) {
    ...
    override fun deliverTreatment(detailedBolusInfo: DetailedBolusInfo): PumpEnactResult {
        ...
        if (detailedBolusInfo.insulin > 0 || carbs > 0) connectionOK = danaPENService?.bolus(detailedBolusInfo.insulin, carbs.toInt(), carbTimeStamp, t) == true
        val result = instantiator.providePumpEnactResult()
    ...
    override fun setTempBasalPercent(percent: Int, durationInMinutes: Int, profile: Profile, enforceNew: Boolean, tbrType: PumpSync.TemporaryBasalType): PumpEnactResult {
        ...
        val connectionOK: Boolean = if (durationInMinutes == 15 || durationInMinutes == 30) {
            danaPENService?.tempBasalShortDuration(percentAfterConstraint, durationInMinutes) == true
        } else {
            val durationInHours = max(durationInMinutes / 60, 1)
            danaPENService?.tempBasal(percentAfterConstraint, durationInHours) == true
        }
    ...
    @Synchronized private fun setHighTempBasalPercent(percent: Int): PumpEnactResult {
        val result = instantiator.providePumpEnactResult()
        val connectionOK = danaPENService?.highTempBasal(percent) == true
    ...
    override fun setExtendedBolus(insulin: Double, durationInMinutes: Int): PumpEnactResult {
        ...
        val connectionOK = danaPENService?.extendedBolus(insulinAfterConstraint, durationInHalfHours) == true
    ...
    @Synchronized override fun cancelExtendedBolus(): PumpEnactResult {
        if (danaPump.isExtendedInProgress) {
            danaPENService?.extendedBolusStop()
