...
class BLEComm @Inject internal constructor(
    ...
    private val danaPENMessageHashTable: DanaPENMessageHashTable,
    private val danaPump: DanaPump,
    private val danaPENPlugin: DanaPENPlugin,
    ...
    private var processedMessage: DanaPENPacket? = null
    ...
    fun disconnect(from: String) {
        ...
        if (!encryptedDataRead && encryptedCommandSent && encryption == EncryptionType.ENCRYPTION_BLE5) {
            ...
            val lastClearRequest = preferences.get(DanaLongKey.DanaPenLastClearKeyRequest)
            if (lastClearRequest != 0L && dateUtil.isOlderThan(lastClearRequest, 5)) {
                ...
                preferences.put(DanaLongKey.DanaPenLastClearKeyRequest, dateUtil.now())
            }
        }
        if (!encryptedDataRead && encryptedCommandSent && encryption == EncryptionType.ENCRYPTION_RSv3) {
            ...
            if (lastClearRequest != 0L && dateUtil.isOlderThan(lastClearRequest, 5)) {
                ...
                preferences.remove(DanaString2Key.DanaPenV3RandomParingKey, danaPENPlugin.mDeviceName)
                preferences.remove(DanaString2Key.DanaPenV3ParingKey, danaPENPlugin.mDeviceName)
                preferences.remove(DanaString2Key.DanaPenV3RandomSyncKey, danaPENPlugin.mDeviceName)
                ToastUtils.errorToast(context, R.string.invalidpairing)
                danaPENPlugin.changePump()
            } else if (lastClearRequest == 0L) {
                aapsLogger.error(LTag.PUMPBTCOMM, "Clearing pairing keys postponed")
    ...
    private val mGattCallback: BluetoothGattCallback = object : BluetoothGattCallback() {
        ...
        override fun onCharacteristicRead(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) {
            ...
            // aapsLogger.debug(LTag.PUMPBTCOMM, "onCharacteristicRead: " + DanaPEN_Packet.toHexString(characteristic.value))
            readDataParsing(characteristic.value)
        }

        override fun onCharacteristicChanged(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
            // for v3 after initial handshake it's encrypted - useless
            // aapsLogger.debug(LTag.PUMPBTCOMM, "onCharacteristicChanged: " + DanaPEN_Packet.toHexString(characteristic.value))
            readDataParsing(characteristic.value)
        }

        override fun onCharacteristicWrite(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) {
            // for v3 after initial handshake it's encrypted - useless
            // aapsLogger.debug(LTag.PUMPBTCOMM, "onCharacteristicWrite: " + DanaPEN_Packet.toHexString(characteristic.value))
            Thread {
    ...
    private fun writeCharacteristicNoResponse(characteristic: BluetoothGattCharacteristic, data: ByteArray) {
        Thread(Runnable {
            ...
            //aapsLogger.debug("writeCharacteristic:" + DanaPEN_Packet.toHexString(data))
    ...
    private fun addToReadBuffer(buffer: ByteArray) {
        //log.debug("addToReadBuffer " + DanaPEN_Packet.toHexString(buffer));
    ...
    private fun readDataParsing(receivedData: ByteArray) {
        //aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< readDataParsing " + DanaPEN_Packet.toHexString(receivedData))
        ...
        val incomingBuffer =
            if (isConnected && (encryption == EncryptionType.ENCRYPTION_RSv3 || encryption == EncryptionType.ENCRYPTION_BLE5))
                bleEncryption.decryptSecondLevelPacket(receivedData).also {
                    ...
                    preferences.put(DanaLongKey.DanaPenLastClearKeyRequest, 0L)
                }
            else receivedData
    ...
    private fun processConnectResponse(decryptedBuffer: ByteArray) {
        ...
        if (decryptedBuffer.size == 4 && decryptedBuffer[2] == 'O'.code.toByte() && decryptedBuffer[3] == 'K'.code.toByte()) {
            aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__PUMP_CHECK (OK)" + " " + DanaPENPacket.toHexString(decryptedBuffer))
            encryption = EncryptionType.ENCRYPTION_DEFAULT
            danaPump.ignoreUserPassword = false
            // Grab pairing key from preferences if exists
            val pairingKey = preferences.get(DanaString2Key.DanaPenParingKey, danaPENPlugin.mDeviceName)
            ...
            // grab randomSyncKey
            preferences.put(DanaString2Key.DanaPenV3RandomSyncKey, danaPENPlugin.mDeviceName, String.format("%02x", decryptedBuffer[decryptedBuffer.size - 1]))

            if (danaPump.hwModel == 0x05) {
                aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__PUMP_CHECK V3 (OK)" + " " + DanaPENPacket.toHexString(decryptedBuffer))
                // Dana RS Pump
                sendV3PairingInformation()
            } else if (danaPump.hwModel == 0x06) {
                aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__PUMP_CHECK V3 EASY (OK)" + " " + DanaPENPacket.toHexString(decryptedBuffer))
                // Dana RS Easy
                sendEasyMenuCheck()
            }
            // response OK BLE5
            ...
            danaPump.hwModel = decryptedBuffer[5].toInt()
            danaPump.protocol = decryptedBuffer[7].toInt()
            val pairingKey = DanaPENPacket.asciiStringFromBuff(decryptedBuffer, 8, 6) // used while bonding
            if (decryptedBuffer[8] != 0.toByte())
                preferences.put(DanaString2Key.DanaPenBle5PairingKey, danaPENPlugin.mDeviceName, pairingKey)

            val storedPairingKey = preferences.get(DanaString2Key.DanaPenBle5PairingKey, danaPENPlugin.mDeviceName)
            ...
            aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__PUMP_CHECK (PUMP)" + " " + DanaPENPacket.toHexString(decryptedBuffer))
            mSendQueue.clear()
            ...
            aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__PUMP_CHECK (BUSY)" + " " + DanaPENPacket.toHexString(decryptedBuffer))
            mSendQueue.clear()
            ...
            aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__PUMP_CHECK (ERROR)" + " " + DanaPENPacket.toHexString(decryptedBuffer))
            mSendQueue.clear()
    ...
    private fun processPasskeyCheck(decryptedBuffer: ByteArray) {
        aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__CHECK_PASSKEY" + " " + DanaPENPacket.toHexString(decryptedBuffer))
    ...
    private fun sendV3PairingInformation() {
        val randomPairingKey = preferences.get(DanaString2Key.DanaPenV3RandomParingKey, danaPENPlugin.mDeviceName)
        val pairingKey = preferences.get(DanaString2Key.DanaPenV3ParingKey, danaPENPlugin.mDeviceName)
        if (randomPairingKey.isNotEmpty() && pairingKey.isNotEmpty()) {
            ...
            val randomSyncKey = preferences.get(DanaString2Key.DanaPenV3RandomSyncKey, danaPENPlugin.mDeviceName)
    ...
    private fun processEncryptionResponse(decryptedBuffer: ByteArray) {
        aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__TIME_INFORMATION " + DanaPENPacket.toHexString(decryptedBuffer))
    ...
    private fun processPairingRequest(decryptedBuffer: ByteArray) {
        aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__PASSKEY_REQUEST " + DanaPENPacket.toHexString(decryptedBuffer))
        if (decryptedBuffer[2] != 0x00.toByte()) {
    ...
    private fun processPairingRequest2(decryptedBuffer: ByteArray) {
        aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + "ENCRYPTION__PASSKEY_RETURN " + DanaPENPacket.toHexString(decryptedBuffer))
        // Paring is successful, sending time info
        rxBus.send(EventDanaPENPairingSuccess())
        sendTimeInfo()
        val pairingKey = byteArrayOf(decryptedBuffer[2], decryptedBuffer[3])
        // store pairing key to preferences
        preferences.put(DanaString2Key.DanaPenParingKey, danaPENPlugin.mDeviceName, DanaPENPacket.bytesToHex(pairingKey))
        aapsLogger.debug(LTag.PUMPBTCOMM, "Got pairing key: " + DanaPENPacket.bytesToHex(pairingKey))
    }
    ...
    fun sendMessage(message: DanaPENPacket) {
        ...
        if (bluetoothGatt == null) {
            aapsLogger.debug(LTag.PUMPBTCOMM, ">>>>> IGNORING (NOT CONNECTED) " + message.friendlyName + " " + DanaPENPacket.toHexString(command) + " " + DanaPENPacket.toHexString(params))
            return
        }
        ...
        if (message is DanaPENPacketEtcKeepConnection && !message.isReceived)
    ...
    private fun processMessage(decryptedBuffer: ByteArray) {
        ...
        val receivedCommand = DanaPENPacket(injector).getCommand(decryptedBuffer)
        val message: DanaPENPacket? = if (originalCommand == receivedCommand) {
            // it's response to last message
            processedMessage
        } else {
            // it's not response to last message, create new instance
            danaPENMessageHashTable.findMessage(receivedCommand)
        }
        if (message != null) {
            aapsLogger.debug(LTag.PUMPBTCOMM, "<<<<< " + message.friendlyName + " " + DanaPENPacket.toHexString(decryptedBuffer))
        ...
        } else aapsLogger.error(LTag.PUMPBTCOMM, "Unknown message received " + DanaPENPacket.toHexString(decryptedBuffer))
    }
